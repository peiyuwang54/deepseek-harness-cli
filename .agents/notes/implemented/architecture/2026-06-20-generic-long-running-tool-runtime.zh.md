...
- `@deepseek-ai/dsh-command-jobs` 通过 `ctx.commands` 向用户公开不消费输出的 `/ps` 与异步 `/stop` 控制。
...
如果生产方加载时没有任何任务控制器，调用方就能启动无法收集或停止的工作。因此，`dsh-tool-jobs` 与 `dsh-command-jobs` 会在各自整个生命周期内调用 `attachController()`；没有附加控制器时，`start()` 会在生产方开始执行前失败。模型工具仍由 Agent preset 选择，而随附 base 会全局挂载用户命令，使交互式会话始终可以列出或停止自己的工作。该检查发生在启动时而非插件加载时，因为兄弟插件可能并发激活。自定义控制器可以自行附加，无需让注册表了解工具或命令名称。
...
单元覆盖固定预检原子性、按 kind 分配的 id、按确切 owner 与无 owner 桶执行的准入、`stopping` 占位、终态释放、输出上限的校验与投影、完整结果的 UTF-8 字节上限、流式与最终读取、等待超时与中止竞态、取消、首次结果优先的结算、监听器隔离、通知压制、所有者隔离、陈旧的所有者实例、所有者清理、服务资源销毁和无控制器防线。命令与 Loader 组装覆盖固定调用者可见的活动任务过滤、不消费输出的 `/ps`、异步 `/stop`、取消 hook 失败和 controller disposal。生产方测试覆盖 bash 进程映射、subagent 启动取消、终止映射与释放。快照覆盖固定控制工具 schema、提示词指导、一条 TUI 后台命令路径，以及一条组合完整的 ACP 路径：配置上限会拒绝第二个真实后台 Bash 任务，并给出 `job_kill` 恢复动作。
