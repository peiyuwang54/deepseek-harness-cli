...
The `headless` product contract is one local task with text or JSONL output, a success-sensitive exit code, empty stderr on successful text runs, and no listening port. A composition containing Workspace Host services, ApiProxy, HTTP, the Web runtime, or browser plugins contradicts that contract and makes local completion depend on an unrelated transport tree.
...
`headless-runner` is a direct core entry point. After Loader settlement, it reads `ctx.agentDefaultModel.currentSelection()`, creates or resumes one Agent through `ctx.agents`, installs that `ModelSelection` and the selected Agent preset in the Agent scope, waits for startup quiescence, anchors the Session sequence, submits one ordinary user message with any admitted images, and waits for quiescence again. It awaits `ctx.sessions.flush` before deriving the final result and `turn/end` reason, and requests bounded launcher shutdown with exit 0 exactly when the reason is `completed` and any required structured capture exists. Text mode writes the final result plus one newline to stdout; JSON mode projects live Session events into `thread.*`, `turn.*`, and `item.*` JSONL. A terminal `error` reason writes its durable code and message to stderr in text mode; unexpected failures use the selected output framing and exit 1.

The [`deepseek exec` automation decision](../feature/2026-08-18-noninteractive-exec-automation.md) owns resume selection, ephemeral sessions, images, structured output, JSONL fields, result files, and command-line permission shortcuts.
...
Package tests use the real Session store, tool registry, and Agent registry around a scripted Agent factory to pin idle-to-idle aggregation, JSONL projection, token usage, image ordering, structured capture, resume selection, ephemeral metadata, permission shortcuts, result files, terminal diagnostics, Loader-time disposal, and flush-before-exit ordering. The keyless assembled snapshots drive the headless profile through a replayed tool round trip, record a `user/message` with `source.kind: 'user'`, and expose a terminal model failure on stderr. Built-bin acceptance reaches a mock provider through the published entry and requires final text on stdout, exit 0, and empty stderr. Config-dump acceptance excludes every Host, Web, and Client package from the shipped headless tree; PTY shutdown coverage requires no observation line and bounded disposal.
...
`deepseek exec` and `dsh --profile headless` provide a local Agent task rather than browser observation, Host APIs, or HTTP. Users who need those capabilities choose `dsh web`. Successful text-mode stderr is empty, completion follows durable flush, and a non-ephemeral Session remains available to later invocations. The initial user message records `source.kind: 'user'` and therefore carries no ApiProxy `rpcId`.
