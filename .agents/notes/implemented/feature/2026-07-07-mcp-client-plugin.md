...
Every paginated Tools, Resources, URI Templates, or Prompts listing, including the connectivity probe, tracks every returned continuation cursor. A repeated cursor rejects the operation before another request instead; tool synchronization remains in its fetch phase, so the prior registered generation stays live.
...
- A server cannot hold a listing or connectivity probe open by cycling cursors. The affected operation fails explicitly; callers may retry after the server fixes its pagination state.
