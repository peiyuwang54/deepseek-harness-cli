...
粘贴或拖放的光栅图片是 Web 输入区对持久附件能力的首个应用场景。未发送文件仍是由客户端持有的临时草稿状态。每个富内容接收适配器都会解码其协议块、验证路由能力，并在追加相应消息事件前将完整图片批次交给附件服务。生成结构化图片输出的提供方适配器在追加相应助手块前，也必须持久提交输出。规范用户内容与助手内容只包含角色无关的 `ImageBlock` 引用。
...
Base64 只跨越一次协议边界，并在持久化后丢弃。每个入口都会校验规范 base64 和声明的 MIME 格式，再用完整的已解码批次调用 `AttachmentStore.saveImages()`。该服务负责图片数量、总字节数、单张图片字节数、完整解码后的光栅图片与 MIME 一致性、固有尺寸和解码像素数；它会先校验全部批次成员，再保存任何成员，因此一张畸形图片不会把批次中的有效成员留成无引用对象。随后按提交顺序执行存储提交，以限制完整光栅解码器的内存占用。如果后续存储 I/O 操作失败，调用方不会追加模型可见事件，也不会收到部分引用，但先前的不可变内容寻址对象可能保持无引用状态；第一版将清理留给未来按引用感知的垃圾回收，而不向去重存储添加破坏性回滚。只有每张图片都成功后，入口才会用规范化文本和按协议顺序排列的持久图片块调用 agent。失败时不公开任何附件路径或原始字节。
...
| 接口 | 职责 |
| --- | --- |
| `packages/attachment/attachment` | 不透明附件标识符、图片引用、限制、错误，以及通过 `ctx.attachments` 提供的单张和批量准入。 |
...
| `packages/host/apiproxy` 和 `packages/bundle/base` | 范围狭窄的上传协议、路由模型前置检查、委托给共享批量准入、先持久化再追加事件的顺序、会话授权读取，以及默认 profile 组合。 |
...
已实现的范围包括附件 seam 和共享批量准入、角色无关的图片块、Pi-AI 输入转换、DeepSeek 拒绝、宿主与 TUI 的持久化顺序、Web 上传与读取协议、当前图片限制执行、大小受限的 Web 请求体、内存草稿图片、粘贴与拖放附件栏、用户与助手历史图片渲染、单击预览、压缩处理，以及组装后无需密钥的 Web 覆盖。
...
- 附件与 TUI 测试覆盖写入前校验全部成员、图文混合顺序、本地路径规范化，以及在提示词历史中复用持久引用。
