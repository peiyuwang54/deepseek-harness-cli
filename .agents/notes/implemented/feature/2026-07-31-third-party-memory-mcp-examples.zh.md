...
这些第三方配置仅作为互操作参考；收录不代表 DeepSeek 的认可、推荐、合作关系或持续支持承诺。系统没有记忆预设注册表、提供方专属 DSH 插件、通用记忆服务、安装 UI、迁移层或提供方专属健康检查器。传输故障检测、有界自动重连和手动重载仍由通用 MCP 运行时负责。其他记忆 MCP 服务器可以使用同一份文档中的 stdio 或 Streamable HTTP 配置项。
...
| 检测传输断开、按配置预算重试并支持手动重载 | 是 | 保持独立 HTTP 服务可用 |
| 账户、认证、模型、embedding、存储初始化 | 否 | 是 |
| 提供方数据迁移、存储修复、保留与遗忘 | 否 | 是 |
...
「新会话」是指同一个 Host 中新建的 DSH 会话，不需要重启 Host。通用 MCP 客户端以异步方式发现工具，并会在配置的尝试预算内自动重连。验证会在第一轮之前等待工具出现；预算耗尽后，`/mcp reload <server>` 或重启 Host 会开始恢复。
...
选择一个文件后，模型可以使用提供方发现到的完整 MCP 工具接口；工具 schema 和 token 成本由提供方决定。移除 `--patch` 就会移除记忆服务器。用户直接接受各上游的许可证、数据政策、云服务费用和运维模式。
