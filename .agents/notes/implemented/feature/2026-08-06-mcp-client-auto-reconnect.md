...
- After final failure or with reconnect disabled, the plugin stays loaded with no (or failing) tools until [manual runtime reload](2026-08-18-mcp-runtime-status-and-reload.md), HMR, or restart — deliberate and logged, so a chronically broken server cannot restart forever.
