...
- [`scripts/exe-build/pipeline.ts`](../../../../scripts/exe-build/pipeline.ts) — the `ExeBuild` pipeline: `--targets` parsing, the per-target `pkg --sea` invocation, asset-coverage verification, `prepareNativePty`, and the macOS `-spawn-helper` packaging.
