...
pkg embeds only files matching `ASSET_GLOBS` into the executable's `/snapshot/` filesystem. Besides Cordis's runtime code imports, the composed application reads data files at boot — every bundle's `cordis.patch.yml` overlay, `@deepseek-ai/dsh`'s shipped `config/agent-presets/` tree, and the web frontend dist — so the globs cover those as well (`node_modules/**/*.yml`, the config tree's `.md` files, `node_modules/@deepseek-ai/dsh-web-frontend/dist/**/*`). The pipeline verifies coverage after staging: every staged `dsh.bundle.patch` overlay and every file a product's `requiredAssets` globs select must match an `ASSET_GLOBS` entry, and each `requiredAssets` glob must match at least one staged file, so an input a skipped build never produced (for example the frontend dist) fails the build instead of the user's first run.
