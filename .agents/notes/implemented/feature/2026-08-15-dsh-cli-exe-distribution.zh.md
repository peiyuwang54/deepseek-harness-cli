...
- **build** 按目标运行：不可变安装、Linux 上 node-pty manylinux 2.28 重建、`scripts/build-dsh-cli-exe.ts --targets=<target>`、Linux 上 GLIBC ≤ 2.28 检查、macOS 部署目标检查、`--version` 冒烟（须等于发布版本）、上传产物。manylinux 容器按绝对的 `$RUNNER_TEMP/setup-pnpm` 路径挂载 pnpm action 目录，因为 node-gyp 会把该路径记录进生成的 Makefile。macOS 检查同时接受 Apple 工具链在不同架构上生成的 `LC_BUILD_VERSION` 与旧式 `LC_VERSION_MIN_MACOSX` load command。
