...
pkg 只把匹配 `ASSET_GLOBS` 的文件嵌入可执行文件的 `/snapshot/` 文件系统。除 Cordis 的运行时代码导入外，组合后的应用在启动时还会读取数据文件——每个 bundle 的 `cordis.patch.yml` overlay、`@deepseek-ai/dsh` 随包发布的 `config/agent-presets/` 树、以及 web 前端 dist——因此通配符同样覆盖它们（`node_modules/**/*.yml`、config 树的 `.md` 文件、`node_modules/@deepseek-ai/dsh-web-frontend/dist/**/*`）。管线在暂存后校验覆盖：每个暂存的 `dsh.bundle.patch` overlay 与产品 `requiredAssets` 通配符选中的每个文件都必须命中一条 `ASSET_GLOBS`，且每条 `requiredAssets` 通配符至少命中一个暂存文件，这样被跳过的构建所缺失的输入（例如前端 dist）会让构建失败，而不是让用户的首次运行失败。
...
- **build** 按目标运行：不可变安装、Linux 上 node-pty manylinux 2.28 重建、`scripts/build-dsh-cli-exe.ts --targets=<target>`、Linux 上 GLIBC ≤ 2.28 检查、macOS 部署目标检查、`--version` 冒烟（须等于发布版本）、profile 组装冒烟（`tui` 与 `web` 的 `--dump-default-config`，会读取嵌入的每个 bundle overlay——`--version` 在此之前就退出）、上传产物。manylinux 容器按绝对路径挂载 runner 的 Node 工具缓存与 pnpm action 目录，再以源码构建模式重建 `@deepseek-ai/dsh-subprocess-local` 工作区的 `node-pty` 依赖；在容器内重新生成 node-gyp 产物，避免宿主缓存 Makefile 的路径进入 Linux 产物。macOS 检查同时接受 Apple 工具链在不同架构上生成的 `LC_BUILD_VERSION` 与旧式 `LC_VERSION_MIN_MACOSX` load command。
