...
Server executables are still trusted local code outside the agent sandbox, and the CLI does not install them. [Enable and disable state](2026-08-18-mcp-capability-management.md) and [OAuth](2026-08-18-mcp-oauth.md) extend the same catalog without changing its secret-reference rule. Catalog changes require a new process; [runtime reload](2026-08-18-mcp-runtime-status-and-reload.md) reconnects only instances already composed from the current file. Unit tests pin parsing, mutation, permissions, reference resolution, redaction, and generated patches; a built-bin e2e test pins real command dispatch and dump composition. No Session snapshot is added because management is boot-free and creates no transcript event.
