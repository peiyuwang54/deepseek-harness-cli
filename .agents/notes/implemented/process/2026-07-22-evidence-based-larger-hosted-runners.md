...
Repositories may configure repo-restricted x64 larger-runner pools for Ubuntu and Windows through `DSH_CI_HOSTED_LINUX_RUNNER` and `DSH_CI_HOSTED_WINDOWS_RUNNER`. The measured profile uses the configured Ubuntu label for three primary Node 24 jobs and the configured Windows label for the independent native inventory. The [portable runner defaults](2026-08-19-portable-ci-runner-defaults.md) supersede direct organization-label assignment: absent those variables, ordinary pull requests use standard GitHub-hosted labels and conservative concurrency. Workflow concurrency remains bounded because an autoscaling ceiling neither allocates idle machines nor makes repository work scale without limit.

The required primary path is complete on standard GitHub-hosted runners. Configured larger labels select the measured acceleration profile without changing job commands or result semantics. Standard jobs also retain the Node 22.19, Node 26, and Python SDK compatibility contracts, while the [portable recovery boundary](2026-07-23-portable-required-pull-request-ci.md) and [serial reference](2026-07-21-serial-cross-platform-ci-reference.md) own required and reference completeness. `suite=larger-runner-benchmark` compares isolated critical lanes across provisioned sizes, and `suite=consolidated-runner-benchmark` compares whole aggregates. Each benchmark reports its observed processor and memory capacity before running repository work.
...
Within the measured configured-hosted topology, Windows shares one setup across the complete native inventory, while Linux owns the duplicate lint, coverage, and snapshot enforcement. The [dual Windows pull-request topology](2026-08-08-native-windows-pull-request-ci.md) keeps that native job independent and non-blocking; the required Windows path remains Wine on standard Linux capacity.
...
Complete serial Linux, macOS, and Windows references run only when `master` moves. Pull requests use the portable hosted selectors plus standard-hosted compatibility jobs, while other larger-runner sizes run only by manual dispatch.
...
**Require the measured larger-runner profile.** This reduces active time when the configured pools allocate promptly, but it prevents repositories without those labels from producing a verdict. Optional primary-label variables retain the acceleration while the standard topology remains the runnable default.
...
The required topology pays one setup wave per lane and retains no shard selectors. Ordinary pull requests use standard hosted capacity unless repository variables select paid larger Linux or Windows runners; manual benchmarks add other sizes only when remeasurement is useful.
...
Missing or renamed configured labels leave their selected jobs queued. Deleting the primary-label variable restores the standard hosted assignment, while the manual benchmarks retain the measurements used to decide whether the acceleration justifies its repository-external dependency.
