...
The self-hosted serial Linux and Windows standby references run when `master` moves, while the hosted `serial-macos` reference remains disabled. Pull requests use portable hosted selectors plus standard-hosted compatibility jobs, while other larger-runner sizes run only by manual dispatch.
...
The required topology pays one setup wave per lane and retains no workflow-level shard jobs or selectors. Process-local coverage partitions share that setup and workspace. Ordinary pull requests use standard hosted capacity unless repository variables select paid larger Linux or Windows runners; manual benchmarks add other sizes only when remeasurement is useful.
