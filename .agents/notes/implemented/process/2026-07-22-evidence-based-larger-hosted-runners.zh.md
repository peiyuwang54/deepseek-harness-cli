...
在已测量的已配置托管拓扑中，Windows 通过一次设置承载完整原生清单，而重复的 lint、覆盖率和快照强制检查由 Linux 负责。[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.md)使该原生作业保持独立且不阻断；必需的 Windows 路径仍是标准 Linux 容量上的 Wine。
...
只有在 `master` 移动时，才运行完整的 Linux、macOS 和 Windows 串行参考。拉取请求使用可移植托管选择器和标准托管兼容性作业，其他大型 runner 规格仅通过手动触发运行。
...
必需拓扑中的每个通道只承担 1 轮设置开销，且不保留分片选择器。普通拉取请求使用标准托管容量，除非仓库变量选择付费的大型 Linux 或 Windows runner；只有在重新测量有价值时，手动基准测试才会加入其他规格。
