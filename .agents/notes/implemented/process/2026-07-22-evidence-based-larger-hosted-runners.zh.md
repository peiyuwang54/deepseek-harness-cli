...
仓库可通过 `DSH_CI_HOSTED_LINUX_RUNNER` 和 `DSH_CI_HOSTED_WINDOWS_RUNNER` 配置仅限本仓库使用的 Ubuntu 和 Windows x64 大型 runner 池。已测量配置使用已配置 Ubuntu 标签执行三个主 Node 24 作业，使用已配置 Windows 标签执行独立的原生清单。[可移植 runner 默认值](2026-08-19-portable-ci-runner-defaults.md)取代了对组织标签的直接分配：未设置这些变量时，普通拉取请求使用 GitHub 托管的标准标签和保守并发度。工作流并发仍设有边界，因为自动扩缩容上限既不会分配闲置机器，也不意味着仓库工作可以无限扩展。

必需主路径在 GitHub 托管的标准 runner 上是完整的。已配置的大型标签会选中已测量加速配置，且不改变作业命令或结果语义。标准作业还保留 Node 22.19、Node 26 和 Python SDK 兼容性约定，而[可移植恢复边界](2026-07-23-portable-required-pull-request-ci.md)与[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.md)分别拥有必需完整性和参考完整性。`suite=larger-runner-benchmark` 比较已预配规格上相互独立的关键通道，`suite=consolidated-runner-benchmark` 则比较完整聚合流程。每项基准测试都会先报告实测的处理器和内存容量，再运行仓库工作。
...
Linux 主流程使用 3 个相互独立的作业。覆盖率单独运行，并按与主机匹配的显式进程上限在该作业内划分插桩工作；静态调度器负责不消费生成输出的源码和文档门禁。第三个作业负责唯一一次 Linux 构建，随后让 lint、Node 24 运行时兼容性、依赖构建产物的快照、文档类型检查和所有产物消费方基于该目录树启动。这种[消费方独立构建](2026-07-30-independent-ci-consumer-build.md)使 3 个作业都能立即请求 runner，而无需重复编译或传输仅供本次运行使用的产物。生成的 NodeNext 消费方目录不会纳入 Oxlint 的文件发现范围，因为这些进程重叠执行时，产物检查会删除这些目录。pnpm store 会得到恢复，但缓存上传不会进入拉取请求关键路径；Oxlint 没有由仓库管理的结果缓存。性能报告采用每个作业从 `startedAt` 到 `completedAt` 的区间；runner 排队延迟是容量证据，而非仓库执行时间。
...
[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.md)把必需的构建与生产网站判定保留在标准托管 Linux 上的 Wine 中。独立且不阻断的原生作业通过一次 Windows 设置共同执行工作区构建、生产网站验证、受支持源码覆盖率与完整的可移植性清单。重复的静态检查、文档、包、构建产物、lint 与快照检查由 Linux 提供阻断性判定，原生聚合流程则保留这些观测性检查。
...
自托管的 Linux 与 Windows 串行热备参考在 `master` 移动时运行，托管的 `serial-macos` 参考仍处于禁用状态。拉取请求使用可移植托管选择器和标准托管兼容性作业，其他大型 runner 规格仅通过手动触发运行。
...
**要求使用已测量的大型 runner 配置。** 当已配置池能及时分配时，这会缩短活动时间，但会阻止没有这些标签的仓库产出判定。可选的主选标签变量保留加速能力，同时让标准拓扑保持为可运行默认值。
...
必需拓扑中的每个通道只承担 1 轮设置开销，且不保留工作流级分片作业或选择器。进程本地 coverage 分区共享这轮设置与同一个工作区。普通拉取请求使用标准托管容量，除非仓库变量选择付费的大型 Linux 或 Windows runner；只有在重新测量有价值时，手动基准测试才会加入其他规格。
...
已配置标签缺失或改名时，其所选作业会持续排队。删除主选标签变量会恢复标准托管分配，而手动基准测试保留用于判断加速收益是否值得其仓库外部依赖的测量。
