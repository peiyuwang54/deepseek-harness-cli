...
标准 Linux 路径使用与标准托管机器相匹配的并发度：两个常规门禁 worker、五个覆盖率 worker，以及四个快照任务。覆盖率拆分会把四个 worker 分配给插桩后的多项目运行，把一个 worker 分配给未插桩的重型测试。在标准四核 runner 上，这两条门禁串行运行，使插桩 worker 在收集 V8 覆盖率时独占机器。阈值清单显式排除剩余行为必须由组装快照覆盖的交互式终端、进程组合和浏览器生命周期路径；它们的聚焦单元测试和快照作业仍是必需门禁。已配置的 Linux 托管标签保留大型 runner 设置，自托管路径保留共享虚拟机设置。独立的 [CI 故障切换手册](2026-07-26-ci-failover-runbook.md)继续拥有故障路由和 Dependabot 排除规则；故障切换优先于主托管标签。
