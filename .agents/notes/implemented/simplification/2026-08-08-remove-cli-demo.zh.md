...
彻底删除 `@deepseek-ai/dsh-cli-demo`：包括它的包、bin、解析器、应用插件、输出格式、测试、workspace 引用、生成目录条目和现行文档。不保留别名或兼容包。源码用户通过 `pnpm dsh exec` 或 `pnpm dsh --profile headless` 调用产品命令；输出、诊断、持久化、退出状态和关闭行为均由随附 headless 组合包负责。
...
- **把旧 demo 的 JSON 与 stream-JSON 协议移到 `dsh --profile headless`。** 不予采纳：保留测试机制不足以支持采用该协议。后续的 [`deepseek exec` 自动化决策](../feature/2026-08-18-noninteractive-exec-automation.md)针对明确的自动化需求引入更小且由产品持有的生命周期投影，没有恢复 demo 包或其协议。
...
这是有意为之的破坏性变更。`dsh-cli-demo`、它的 `--output-format` 选项以及对 `@deepseek-ai/dsh-cli-demo/src/cli.ts` 的导入都无法解析。Headless 产品持有独立的 `--json` 生命周期与结构化输出模式；两者都不恢复对已删除 demo 协议的兼容。
...
聚焦的 Loader 冒烟测试在源码模式和由普通 Node 启动的构建模式下覆盖显式组装，快照测试对比其规范 JSONL 和持久化日志，产品验收覆盖 headless profile，文档检查及生成图谱／目录检查则拒绝对已移除包的活跃引用。冻结的 Agent Note 归档保留为历史证据，不会被重写。
