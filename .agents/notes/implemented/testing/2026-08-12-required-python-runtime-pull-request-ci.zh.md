...
构建器会在创建可执行文件前部署一个仅含生产依赖的 JSON-RPC 工作区闭包。这个裁剪后的闭包会有意排除 TUI 等无关产品界面，因此部署命令只在该处允许存在未使用的工作区补丁，而仓库的普通安装仍保留 pnpm 对未使用补丁的严格校验。这样，被排除界面所拥有的补丁不会阻断 SDK 载体，同时也不会削弱完整工作区检查。
...
部署专用的未使用补丁放行以运行时闭包验证器继续作为 SDK 载体所含包的权威检查为前提。如果载体所含依赖缺少必要补丁，安装或发布形态运行时验证仍会失败。
