...
支持目标：macOS（`arm64`、`x64`）、Linux（`arm64`、`x64`）与 Windows（`x64`）。`install.sh` 运行在普通 POSIX `sh` 上，需要 `curl`、`tar` 与一个 sha256 工具（macOS 用 `shasum`，Linux 用 `sha256sum`）。两个安装器都会提供 `deepseek` 与 `dsh` 命令；`install.ps1` 使用 `Get-FileHash` 校验下载文件。
