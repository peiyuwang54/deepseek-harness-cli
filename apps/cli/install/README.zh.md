# deepseek-harness-cli — curl 与 PowerShell 安装器
...
支持目标：macOS（`arm64`、`x64`）、Linux（`arm64`、`x64`）与 Windows（`x64`）。`install.sh` 运行在普通 POSIX `sh` 上，需要 `curl`、`tar` 与一个 sha256 工具（macOS 用 `shasum`，Linux 用 `sha256sum`）。`install.ps1` 在 Windows PowerShell 中运行，用 `Get-FileHash` 校验，并写出 `dsh.cmd` 与 `deepseek.cmd`。
...
完成后请重启 shell（或运行它打印的 `export PATH=…` 那一行），然后运行 `deepseek`。
