...
进程关闭时，插件树最多有 5 秒完成 dispose。正常的 `ctx.appExit` 请求会立即启动这段有界等待；其 fiber disposer 会停在启动屏障，等待 profile 启动结算，并让配置专用 HMR 与两个精确路径用户 patch 监视器的建立尝试完成，或因已有退出请求而明确跳过，再 dispose 根节点。因此，即使应用快速显示帮助或报告用法错误并退出，进程也不会遗留 Chokidar 句柄。首次收到 `SIGINT` 或 `SIGTERM` 时会开始相同的优雅排空：`SIGTERM` 是监督进程发出的常规停止请求，在所有运行模式下都以 0 退出；`SIGINT` 则报告 130。即使启动或 dispose 仍未完成，第二次收到信号也会立即强制退出。如果一次性运行在正常结束时已经卡在 dispose 阶段，第一次按下 `Ctrl+C` 就会直接升级为强制退出，而不会被忽略。

所有模式都将运行命令时所在的目录作为默认 workspace 根目录，以 65,536 字节渲染预算加载适用的 `AGENTS.md` 或 `CLAUDE.md` 指令，并使用内存 SQLite 会话内容索引。每次启动 profile 时，系统都会为 profile 与 home 两个 `cordis.patch.yml` 配置层的有效变更打开精确路径监视器，并以事务方式重新应用。只有在 HMR 服务与两个监视器建立尝试完成结算后，启动屏障才会允许正常的一次性 disposer 继续；随后，有界关闭会 dispose 这些监视器。
