...
import { clientArtifactPaths, readClientBuildRecord } from '../../../scripts/client-build-environment.ts'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const originalClientArtifacts = await Promise.all(clientArtifactPaths(REPO_ROOT).map(async path => [
    path,
  ...
  try {
    ...
    await Promise.all(clientArtifactPaths(REPO_ROOT)
      .filter(path => !originalClientArtifactPaths.has(path))
      .map(async path => rm(join(REPO_ROOT, path), { force: true })
        .catch((error: unknown) => failures.push(error))))
    await Promise.all(originalClientArtifacts.map(async ([path, content]) => {
      const absolute = join(REPO_ROOT, path)
      await mkdir(dirname(absolute), { recursive: true }).catch((error: unknown) => failures.push(error))
      await writeFile(absolute, content).catch((error: unknown) => failures.push(error))
    }))
