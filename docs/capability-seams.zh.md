...
flowchart LR
  ...
  pkg_mcp["mcp"]
  svc_mcp["ctx.mcp<br/>MCP runtime status and lifecycle control"]
  ...
  svc_subagentWorktrees["ctx.subagentWorktrees<br/>Isolated Git worktree manager"]
  ...
  pkg_subagent_worktree --> svc_subagentWorktrees
  ...
  svc_subagentWorktrees --> pkg_subagent_in_process_driver
  svc_subagentWorktrees --> pkg_tui
...
| `ctx.mcp` | `seam` | [`mcp`](../packages/mcp/mcp) | [`mcp-client`](../packages/mcp/mcp-client) | [`tui`](../packages/ui/tui) | - | 具体 MCP 客户端发布由 effect 管理的连接快照与立即重载控制；用户诊断消费该 seam，但不持有传输或工具注册。 |
...
| `ctx.subagentWorktrees` | `seam` | [`subagent-worktree`](../packages/subagent/subagent-worktree) | - | [`subagent-in-process-driver`](../packages/subagent/subagent-in-process-driver), [`tui`](../packages/ui/tui) | - | Host 侧管理器创建持久化的子分支与检出目录；进程内编码子代理将该检出用作会话 cwd，TUI 提供显式的状态、合并和丢弃控制。 |
| `ctx.jobs` | `seam` | [`jobs`](../packages/jobs/jobs) | [`jobs-local`](../packages/jobs/jobs-local) | [`tool-bash`](../packages/shell/tool-bash), [`tool-terminal`](../packages/terminal/tool-terminal), [`tool-subagent`](../packages/subagent/tool-subagent), [`tool-jobs`](../packages/jobs/tool-jobs), [`command-jobs`](../packages/jobs/command-jobs) | - | 生产方（后台 bash、PTY 发送和 subagent 委派）登记正在运行的工作；tool-jobs 是面向模型的控制器，command-jobs 提供按所有者隔离的用户 `/ps` 与 `/stop`，jobs-local 则是进程本地注册表。 |
