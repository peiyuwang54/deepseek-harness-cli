...
Requires: `agents` · `sessions` · `approval` · `commands` · `userQuestions` · `tools` · `llm` · `systemPrompt` · `tokenMeter` · `tuiPrompt`
...
- `@deepseek-ai/dsh-command-jobs` — requires `commands` · `jobs` ([`packages/jobs/command-jobs/src/index.ts`](../packages/jobs/command-jobs/src/index.ts))
