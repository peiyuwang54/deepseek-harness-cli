...
- `@deepseek-ai/dsh-terminal-cli` — requires `terminalCliStartup` · `agentDefaultModel` · `agents` · `sessions` · `sessionPersistence` · `tools` · `commands` · `userQuestions` · `approval` ([`packages/bundle/terminal-cli/src/index.ts`](../packages/bundle/terminal-cli/src/index.ts))
