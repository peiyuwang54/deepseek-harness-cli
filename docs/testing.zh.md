...
- **覆盖率门禁**（`pnpm run test:coverage`）：门禁级运行，在应用 `vitest.config.ts` 中显式列出的交互式运行时豁免后，对 `packages/*/*/src` 按文件 100% 覆盖；终端、进程组合和浏览器生命周期的缺口仍由对应的组装快照门禁强制执行。阈值清单中未覆盖的行往往是门禁正确标记出的死代码（应删除），而非需要补写的测试。行覆盖率是必要条件，但永远不是充分条件：它证明行被执行过，不证明功能按交付预期工作。`packages/shell/pwsh-local/src` 的按文件 100% 覆盖需要真实的 `pwsh`：缺少它时其执行器套件会自动跳过，`vitest.config.ts` 会豁免该文件以使无 pwsh 的主机保持绿色，而 CI runner 自带 pwsh，仍按完整标准执行门禁。
