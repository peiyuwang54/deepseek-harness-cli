...
必须新建 DSH 会话，但不需要重启 Host。通用客户端会在配置的尝试预算内自动重连崩溃的子进程或断开的 HTTP 传输；恢复期间仍会显示最后一个正常的工具世代，但传输不可用时调用可能失败。预算耗尽后，请使用 `/mcp reload <server>` 或重启 Host。初始发现过程是异步的，因此发送第一条验证提示词前，请等待提供方的 `mcp__...` 工具出现。
