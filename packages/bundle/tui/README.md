...
[`src/startup.ts`](src/startup.ts) owns `dsh tui`'s `--resume`, `--yolo`, and `--help`. A successful interactive launch publishes one immutable `tuiStartup.identity`: either a fresh `main-session-<uuid>` or the requested persisted session. `--yolo` additionally publishes explicit full-access intent; before the Agent is published, the runner resolves the deployment's preset that combines `danger-full-access` with `never` and pins it into the session through `ctx.permissionPresets`. A deployment without that preset fails before presentation mounts. The permission command registry intentionally contains no `/yolo`; live changes continue through `/permissions`. Startup also supplies the renderer's main-session identity and printable resume command. Help remains usable through a pipe; an otherwise successful launch without TTY stdin and stdout requests a failing bounded exit before the dependency-heavy runner activates.
