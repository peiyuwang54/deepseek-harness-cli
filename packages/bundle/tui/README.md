...
[`src/startup.ts`](src/startup.ts) owns the direct `deepseek` command's `--resume`, repeatable `--add-dir`, `--full-auto`, `--yolo`, `--dangerously-bypass-approvals-and-sandbox`, and `--help` flags; `dsh tui` remains the compatibility spelling. A successful interactive launch publishes one immutable `tuiStartup.identity`: either a fresh `main-session-<uuid>` or the requested persisted session. Each `--add-dir` resolves against the session cwd, validates as an existing directory, and appends to the session's durable additional writable-root set before the Agent is published; resumed sessions retain earlier roots and may add more. `--full-auto` pins the configured `workspace-write` + `never` preset, while both unrestricted spellings pin `danger-full-access` + `never`, before the Agent is published. Missing presets and invalid roots fail before presentation mounts, and the two unrestricted startup modes cannot be combined. The command registry intentionally contains no session-level startup shortcuts; live permission changes continue through `/permissions`. Startup also supplies the renderer's main-session identity and printable `deepseek --resume` command. Help remains usable through a pipe; an otherwise successful launch without TTY stdin and stdout requests a failing bounded exit before the dependency-heavy runner activates.
