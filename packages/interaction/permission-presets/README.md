...
Two optional children ship the product surfaces over the same service: a `permissions` session-projection unit (`src/types.ts` declares the key; the unit folds the three whole-value knob events and views the select — table options plus a current-only `custom` — over the composition defaults) and command entries. `/permissions` reports or selects a named preset. `/yolo` is the explicit high-risk shortcut for the table entry that combines `danger-full-access` with `never`; it rejects arguments, reports when the deployment has no such entry, and names the previous selectable preset as the recovery command. Each child activates only when its registry (`ctx.sessionProjections` / `ctx.commands`) is composed.
