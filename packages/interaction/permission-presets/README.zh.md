...
两个可选子功能在同一服务之上提供产品界面：`permissions` 会话投影单元（`src/types.ts` 声明该 key；单元以组合默认值为基础折叠三个全量值可调参数事件，并生成选择器视图，其中包含表内选项和仅作当前值的 `custom`）与命令条目。`/permissions` 报告或选择一个具名 preset。`/yolo` 是显式的高风险快捷方式，目标是表中组合 `danger-full-access` 与 `never` 的条目；它拒绝参数，在部署没有该条目时报告不可用，并把之前可选择的 preset 作为恢复命令。每个子功能仅在其注册表（`ctx.sessionProjections` / `ctx.commands`）被组合时激活。
