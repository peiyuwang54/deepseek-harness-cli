...
Human-facing background-job controls over [`ctx.jobs`](../jobs/README.md). The plugin registers `/ps` and `/stop` through [`ctx.commands`](../../interaction/commands/README.md), so every composed command adapter discovers the same owner-fenced operations without a model turn. Loading it also attaches a job controller; producers may therefore start work for an Agent even when that Agent's preset omits the model-facing [`job_*` tools](../tool-jobs/README.md).
...
- **Cancellation is asynchronous** — `/stop` requests every running cancellation but does not wait for producer settlement.
