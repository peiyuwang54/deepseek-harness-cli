...
基于 [`ctx.jobs`](../jobs/README.md) 的用户侧后台任务控制。该插件通过 [`ctx.commands`](../../interaction/commands/README.md) 注册 `/ps`、`/stop` 和兼容 Codex 的 `/clean` 别名，因此所有已组装的命令适配器无需启动模型轮次，即可发现相同的按所有者隔离操作。加载插件还会附加一个任务控制器；即使某个 Agent 的 preset 未提供面向模型的 [`job_*` 工具](../tool-jobs/README.md)，生产方也可以为该 Agent 启动任务。
...
| `/clean` | 执行与 `/stop` 相同的操作，并使用 `Stopped by /clean.` 作为取消原因。 |
...
- **取消是异步的** —— `/stop` 与 `/clean` 会请求取消全部运行中任务，但不会等待生产方完成结算。
