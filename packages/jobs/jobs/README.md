...
- `adopt(spec): JobId` applies the same preflight to work that is already running, then transfers its hooks into the registry without starting a duplicate operation. A rejection leaves the original foreground owner responsible for the work.
...
- `attachController(name)` declares a job controller for its effect lifetime. `start()` and `adopt()` fail before registration when no attached controller serves the spec's owner.
...
- **Adoption is explicit and in-process** — only a foreground owner that can supply cancellation, settlement, and output hooks may promote existing work; the registry cannot infer those hooks from an arbitrary process.
