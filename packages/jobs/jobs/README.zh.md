...
- `adopt(spec): JobId` 对已经运行的工作执行同样的预检，然后把其 hooks 转交给注册表，不会启动第二份操作。如果被拒绝，原前台 owner 仍负责该工作。
...
- `attachController(name)` 在其 effect 生命周期内声明任务控制器。当没有任何已附加的控制器服务于 spec 的所有者时，`start()` 和 `adopt()` 都会在注册前失败。
...
- **接管是显式且仅限进程内的**：只有能提供取消、结算和输出 hooks 的前台 owner 才能提升现有工作；注册表无法从任意进程推断这些 hooks。
