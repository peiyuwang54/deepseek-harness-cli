...
// match the grant shape. workspace-write requires distinct root-set and
// private-temp identities; pass tempDir: null to disable temp writes.
const sandbox = new AclSandbox({
  writableDirs: workspaceRoots,
  tempDir,
  writeSid: workspaceWriteSid(workspaceRoots),
  tempWriteSid: tempWriteSid(tempDir),
  mode: 'workspace-write',
})
await sandbox.init() // throws on ANY Win32 failure — never spawns unrestricted

const child = sandbox.spawn({ command: 'pwsh', args: ['-NoProfile', '-Command', '...'], cwd: workspaceRoots[0]! })
...
```sh
node runner.js --workspace <dir> [--workspace <dir> ...] --temp <dir> --mode <read-only|workspace-write> [--write-sid <S-1-4-…> --temp-write-sid <S-1-4-…>] -- <argv...>
```
...
**Root-set reuse and temp isolation**: the seam materializes one deterministic root-set SID's ACE STANDING on every root (once per root set per server lifetime, never revoked — it is the reuse cache), then creates a random private temp directory and distinct revocable SID for each live session/root-set pair. It passes both identities as the required `--write-sid`/`--temp-write-sid` pair; the runner verifies the write SID against the complete order-independent root set and neither grants nor revokes (`manageDacls: false`). A fork receives a different temp capability, and a fresh provider gives even the same resumed session a new path and SID, so crash residue is inert litter rather than a collision or inherited capability. Without the pair, `--temp` names a root: an agentless/standalone workspace-write runner creates a random private child, self-manages its temp SID, rewrites TMP/TEMP, and removes the child on exit. A writable root equal to or containing that temp root is rejected before any grant because its inheritable ACE would otherwise authorize every private child; the direct API likewise rejects overlap between any writable root and the actual private temp directory. Re-granting a standing root ACE after a restart is idempotent: `grantWrite` reads the current DACL and skips `SetNamedSecurityInfoW` when the exact ACE already stands. Known cost: the first grant on a large root tree blocks for eager propagation once per root-set identity per machine.
...
- **ACL grants are standing directory mutations.** They persist if the process dies mid-run; workspace ACEs are standing BY DESIGN (never revoked — the reuse cache), temp ACEs are revoked by `dispose()` (`init()` also revokes an already-applied temp grant when a later step fails). The POC's documented manual cleanup (`icacls <dir> /remove '*S-1-4-…'`) fails on this platform with `ERROR_NONE_MAPPED` (1332) — revoke through this module instead. An unclean shutdown needs no self-healing for the workspace ACEs: the derived SID re-hits each standing ACE on the next provision, and the complete root set determines one stable identity.
...
- **One write allowlist per root set** — the write SID is the unit of the allowlist and identifies the complete sorted canonical root set. Create a distinct sandbox instance when that set changes; the seam does this automatically and never accumulates new roots under an existing SID.
...
- **Grant materialization is an eager full-tree propagation.** `SetNamedSecurityInfoW` on a directory with inheritable ACEs walks every descendant immediately (NOT lazily per access — measured at tens of seconds on large workspace trees). A root-set identity pays this once on each root per machine (lazily at the first confined execution, skipped on later provisions when the exact ACE stands). Private temp directories start empty, so their distinct grant is cheap. If a root is huge, the first confined write on this host is correspondingly slow.
