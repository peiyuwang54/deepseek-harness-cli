/**
 * workspaceWriteSid tests: the writable-root-set identity is deterministic
 * (the same canonical root set always derives the same SID — the property the
...
describe('workspaceWriteSid', () => {
  ...
  it('derives distinct identities for distinct root sets', () => {
    expect(workspaceWriteSid(['C:\\Users\\agent\\repo-a'])).not.toBe(workspaceWriteSid(['C:\\Users\\agent\\repo-b']))
  })

  it('is byte-sensitive: canonical paths are the caller\'s contract', () => {
    expect(workspaceWriteSid(['C:\\Repo'])).not.toBe(workspaceWriteSid(['c:\\repo']))
    expect(workspaceWriteSid(['C:\\Repo\\'])).not.toBe(workspaceWriteSid(['C:\\Repo']))
  })

  it('is order-insensitive and deduplicates an identical root', () => {
    const first = workspaceWriteSid(['C:\\Repo', 'D:\\Shared'])
    expect(workspaceWriteSid(['D:\\Shared', 'C:\\Repo'])).toBe(first)
    expect(workspaceWriteSid(['C:\\Repo', 'D:\\Shared', 'C:\\Repo'])).toBe(first)
  })
})
