...
- **Explicit deletion.** `delete(id)` flushes a live owner, unlinks the exact transcript, syncs directory metadata on POSIX, and removes the session directory only when empty. It never recursively removes a project directory.
