...
- **Explicit deletion.** `delete(id)` flushes a live owner and deletes its `sessions` row; the foreign key cascade removes every associated event row in the same SQLite statement.
