...
`create()` materializes lazily on the first append. Each append batch and each interrupted-turn repair is atomic, while `inspect()` synthesizes recovery closers without mutating storage. `delete(id)` flushes a live owner and deletes its session row and associated events through the foreign-key cascade. Snapshot revisions combine the database identity, materialization incarnation, and per-session mutation counter so unchanged observations remain stable and recreated same-id logs remain distinct.
...
- **No background historical compaction** — normal appends are insert-only; explicit session deletion remains available.
