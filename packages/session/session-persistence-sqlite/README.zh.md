...
`create()` 会延迟到第一次 append 时才实体化。每个 append 批次和中断轮次修复都是原子的，`inspect()` 则只合成恢复 closer，不修改存储。`delete(id)` 会 flush 活动 owner，并通过外键级联删除对应 session 行及其关联事件。快照 revision 组合数据库身份、实体化 incarnation 与逐 session 变更计数器，因此未变化的观察保持稳定，重建的同 id 日志也保持可区分。
...
- **没有后台历史压缩**——普通追加只做插入；显式删除 session 仍然可用。
