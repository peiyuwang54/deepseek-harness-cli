...
describe('SessionPersistenceSqlite schema ownership', () => {
  ...
  it('rejects schema ownership changes observed at mutation time', async () => {
    ...
    const path = await freshDbPath('dsh-sqlite-delete-schema-change-')
    const store = new SqliteStore({ path, journalMode: 'wal', busyTimeoutMs: DEFAULT_BUSY_TIMEOUT_MS })
    const header = meta('delete-schema-change')
    await store.appendBatch(header, [chunk(0)], false)
    const changedDelete = new DatabaseSync(path)
    changedDelete.exec(testSql('set-user-version-16'))
    changedDelete.close()
    await expect(store.deleteStored(header.id)).rejects.toThrow(/schema changed before mutation/)
    expect((await store.list()).map(candidate => candidate.id)).toContain(header.id)
    await store.close()
  })
