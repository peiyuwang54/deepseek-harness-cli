...
| `delete(id, signal?): Promise<void>` | Flush and permanently remove one session. An absent or never-materialized id succeeds; events arriving during a failed deletion are restored, while successful deletion cannot be undone by teardown. The caller immediately disposes a deleted live Session. |
...
Each non-ephemeral `session/event` copies its event into the session controller. An ephemeral Session never receives a controller, does not participate in flush, and is excluded from HMR adoption. The first pending event starts a fixed batching window; later events join without resetting its deadline. The configured `writeBatchMaxDelayMs` bounds this intentional wait, not event-loop, initialization, serialized-operation, or backend latency. Events admitted during a write form a new bounded batch. `session/flush` cancels the wait and is a shared quiescence barrier that drains events admitted while it runs. A background failure is logged once, retains the ordered batch, and pauses automatic retry; a new event starts a fresh window, while explicit flush or backend teardown retries immediately and surfaces a repeated failure.
...
| `deleteStored(id, signal?)` | Permanently remove one stored session; absence succeeds. |
...
- **No retention policy** — callers explicitly delete one session; automatic age, size, and quota pruning remain deployment policy.
