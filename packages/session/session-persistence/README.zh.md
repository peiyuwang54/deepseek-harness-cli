...
| `delete(id, signal?): Promise<void>` | Flush 并永久删除一个会话。id 不存在或从未实体化时仍成功；删除失败时会恢复期间到达的事件，删除成功后 teardown 无法重建日志。调用方会立即 dispose 已删除的活动 Session。 |
...
| `deleteStored(id, signal?)` | 永久删除一个已存储会话；目标不存在时仍成功。 |
...
- **无保留策略**：调用方可显式删除一个会话；按时间、大小或配额自动清理仍属于部署策略。
