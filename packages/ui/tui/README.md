...
While a turn runs, an animated `Deep diving (<elapsed> • esc to interrupt)` row stays at the live conversation tail and uses the durable `turn/start` time. The composer footer separately advertises `Ctrl+B background` and `Esc interrupt`. It is removed when the turn settles; completed steps retain their per-phase timing summary. The right footer continues to show Goal, model, token, context, and queued-session state instead of duplicating the running label.
...
Typing `@` at a token boundary searches files and directories under the session working directory. A bare fuzzy query uses a reusable bounded workspace index; a query containing `/` lists that directory directly, and selecting a folder keeps completion open for descent. Search respects repository `.git/info/exclude`, scoped `.gitignore`, and nested `.ignore` rules by default. Whitespace-bearing paths are inserted as `@"path with spaces"`. Selecting a file inserts only its path and a trailing space: the TUI does not read it, attach hidden context, or replace it with a reference object. When a model-facing `read` tool is registered, the TUI adds one fixed system-prompt instruction telling the model to read an explicit path when its contents are needed.
...
`/rewind` selects a prior human message, then restores the conversation before that message, workspace files from before its first tool call, or both. Conversation rewind creates and switches to a child Session; it never truncates the original log. Workspace checkpoints use an isolated Git object store under `$DSH_HOME/workspace-checkpoints/v1`, not the project's repository, and include tracked plus non-ignored untracked files and symbolic links. Restoring captures the current workspace first, removes only paths known to the shadow history, and leaves ignored files untouched. Admission fails before tool execution for unsupported special files, per-file or aggregate size limits, Git failures, and paths below symbolic-link directories. `/restore` opens the same history with files-only preselected. Direct `!command` execution also checkpoints before the shell starts.

`/rewind` and `/restore` are agent-scoped TUI commands and complete the command set described above.

`/side [message]` opens a temporary side conversation after the parent has completed at least one user turn; `/btw` is its hidden alias. It inherits the latest completed-turn prefix, preset, model, and reasoning effort, but treats prior instructions as reference-only, disables inherited Plan mode and Goal activation, and blocks subagent and workflow tools. The side transcript hides inherited rows and its internal boundary; only `/copy`, `/cwd`, `/diff`, `/export`, `/ide`, `/mention`, `/pwd`, `/raw`, `/status`, and `/usage` remain available. Ctrl+C interrupts active side work first and returns to the unchanged parent when the side is idle. The side Agent is then disposed, and its `ephemeral` Session header and events are never admitted to persistence or `/resume`.
...
`/mcp [list|tools|desc|schema|auth] [server]` groups the MCP-qualified tools visible through the current Agent's scoped tool registry. `list` is the default, `tools` is an explicit alias, `desc` includes normalized descriptions, and `schema` adds parameter schemas; the optional server id narrows any view. `ls` aliases `list`, and `verbose` remains an alias of `desc`. `/mcp auth <server>` prints the boot-free `deepseek mcp auth <server>` flow for browser OAuth. The command does not expose unrelated tools or infer server connection state that the tool registry does not own.
...
`/hooks [verbose]` lists successfully loaded Claude Code and Codex hook bridge configurations from optional `ctx.hooks`. The default view reports each source and runnable-handler total; `verbose` expands lifecycle points, matchers, commands, timeout overrides, and parsed-but-skipped handlers. It is diagnostic and read-only: profile configuration remains responsible for enabling, trusting, disabling, or editing hooks.

`/plugins [verbose] [query]` browses the current profile's live Cordis Loader inventory. The default output is bounded to 20 rows and reports total configured, active, and disabled entries; a query filters by module specifier or Loader id, while `verbose` adds the complete values and root Fiber phase. The browser is read-only. Install a profile package outside chat with `deepseek plugin --profile tui add <package>`; use `remove` or `update` in the same position for those operations, and `dsh` is an equivalent spelling.

`/import` detects compatible local setup from Claude Code and Codex, asks for a source when both are available, then opens a checked multi-select over user/project skills and instructions. `/import claude|codex [all|skills|instructions]` selects a source directly and can run without the item picker. Compatible `SKILL.md` bundles and flat Markdown skills are copied into `.agents/skills`; user instructions go to `$DSH_HOME/AGENTS.md`, while instructions found only under a project's product directory go to the project `AGENTS.md`. Existing destinations are retained, symbolic links and special files are rejected, and root-level project `CLAUDE.md` needs no import because the Harness reads it natively. Settings, plugins, hooks, MCP servers, and chat histories are not copied across products whose formats or lifecycle ownership differ.
...
`/feedback <text>` reuses the global session-feedback command mounted by the base bundle. It records one log-only `feedback/record`, starts no model turn, and acknowledges with the receiving Session, anonymous user id, and current session-sharing policy. The command never claims that an optional telemetry backend delivered or retained the entry.
...
`/language [en|zh|zh-tw|ar|fr|ru|es|ja|ko]` writes the shared `locale.preference` setting and offers English, Simplified Chinese, Traditional Chinese, Arabic, French, Russian, Spanish, Japanese, and Korean terminal copy. Bare `/language` opens the composer-attached selector; TUI changes refresh the welcome dashboard, default composer placeholder, editor footer, live-turn row, credential UI, and Settings surfaces immediately. The browser applies shared English or Simplified Chinese selections and follows its browser-derived language when the stored preference is terminal-only. Model output, tool payloads, custom placeholders, and third-party command copy remain source-authored rather than being machine-translated.

`/workspace` and `/cd` open a searchable selector over the shared durable `ctx.workspaceRegistry`; either command with a `<directory>` canonicalizes and registers that directory first. Choosing a row starts a **fresh** session in that workspace through the optional host-owned `TuiRuntime.handoffWorkspace`. The controller requires an idle agent, checks the directory, flushes the current session, drains input, and releases the UI plus full-screen/mouse terminal modes before handoff. A missing host leaves the current TUI running with a warning; a rejected host restores the terminal and forces a complete frame. It never rewrites the current session's immutable `SessionHeader.cwd` — changing workspace is a process/session handoff, not an in-place metadata mutation. `/pwd` and its `/cwd` alias report the effective directory without changing state and remain available in side conversations.
...
| `fileSearchRespectIgnoreFiles` | `TuiConfig` | `true` | Apply repository `.git/info/exclude`, scoped `.gitignore`, and nested `.ignore` rules |
...
- **File search does not read Git's user-global excludes** — repository `.git/info/exclude`, scoped `.gitignore`, and nested `.ignore` rules apply by default, while `fileSearchRespectIgnoreFiles: false` disables all ignore-file interpretation. Explicit directory exclusions always apply, and directory symlinks are not traversed.
