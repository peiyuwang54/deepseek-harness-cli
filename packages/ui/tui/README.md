...
`/accent [deepseek|cosmic-orange|mist-blue|sage|lavender|deep-blue]` writes the TUI-owned `ui-accent.accent` setting; bare `/accent` opens a composer-attached selector over the shipped hues. The accent repaints the interaction emphasis (prompt, borders, role headers, selection) and, on truecolor terminals, the startup banner gradient and brand ink, while the ANSI fallback stays theme-adaptive on 16-color terminals. The default is `deepseek`, unchanged from the original DeepSeek-blue chrome.
...
The accent hue is selectable through `/accent`: `deepseek` (default), `cosmic-orange`, `mist-blue`, `sage`, `lavender`, and `deep-blue`. Each hue pairs a truecolor 24-bit ink for the accent role and banner gradient with an ANSI 16-color fallback for non-truecolor terminals, so the chrome stays theme-adaptive either way.
