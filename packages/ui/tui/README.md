...
| `theme.truecolor` | `TuiConfig` | process entry detects `COLORTERM`; direct runtime calls use `false` | Enable the 24-bit startup gradient and official-mark ink |
| `theme.leftPrompt` | `TuiConfig` | `${cwd}${git/worktree}${model}${token_meter/cache_hit_rate}${context}` | Left-aligned template on the row above the editor |
| `theme.rightPrompt` | `TuiConfig` | `${queued}` | Right-aligned template on the row above the editor |
| `theme.inputPrompt` | `TuiConfig` | `${symbol} ${indicator}` | Editor first-line prefix template |
| `theme.inputPlaceholder` | `TuiConfig` | `press enter to steer and esc to cancel` | Empty-editor placeholder while the agent runs |
