...
`/side [message]` opens a temporary side conversation after the parent has completed at least one user turn; `/btw` is its hidden alias. It inherits the latest completed-turn prefix, preset, model, and reasoning effort, but treats prior instructions as reference-only, disables inherited Plan mode and Goal activation, and blocks subagent and workflow tools. The side transcript hides inherited rows and its internal boundary; only `/copy`, `/diff`, `/export`, `/ide`, `/mention`, `/raw`, `/status`, and `/usage` remain available. Ctrl+C interrupts active side work first and returns to the unchanged parent when the side is idle. The side Agent is then disposed, and its `ephemeral` Session header and events are never admitted to persistence or `/resume`.
