...
`/archive` is available only on the idle primary Agent. It opens a confirmation card, flushes the Session, adds its id to the durable workspace archive set, and exits only after the archive write succeeds. Archiving hides the Session from active workspace lists but retains its complete log. A custom embedding without `ctx.workspaceRegistry`, a storage failure, or a turn starting during confirmation leaves the current TUI open. Session deletion is not exposed because the persistence service has no deletion API.
