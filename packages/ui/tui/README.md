...
`/mcp [list|tools|desc|schema] [server]` groups the MCP-qualified tools visible through the current Agent's scoped tool registry. `list` is the default, `tools` is an explicit alias, `desc` includes normalized descriptions, and `schema` adds parameter schemas; the optional server id narrows any view. `ls` aliases `list`, and `verbose` remains an alias of `desc`. The command does not expose unrelated tools or infer server connection state that the tool registry does not own.
