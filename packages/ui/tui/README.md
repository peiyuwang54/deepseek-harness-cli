...
A fresh, empty session opens on an adaptive welcome dashboard. Wide terminals center a terminal-native DeepSeek mark and wordmark above cards for the composed agent preset, selected model, and effective permission preset or approval policy. Up to three newest sessions come from the optional session-query service, followed by direct `/model`, `/workspace`, `/settings`, `/resume`, `/help`, and `@` hints; short terminals collapse this to one compact action row. The dashboard contracts to the ordinary transcript header as soon as the first turn starts. Recent-session rows are informative rather than clickable; `/resume` owns searching and validation.
