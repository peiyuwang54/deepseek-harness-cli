...
The compact bottom status bar shows Goal/details state, current model, `↑<uncached input> ↓<output>`, known token-meter context pressure, and queued work on the right. Its default left side is empty, so the working directory and branch do not consume chat width; `/status` retains both diagnostics, and an embedding may opt back in through `theme.leftPrompt`. It also omits idle state and cache rate: live work is shown by the animated conversation-tail status, while cache hit rate remains in the detailed session-statistics row.
...
| `theme.rightPrompt` | `TuiConfig` | `${goal}${details}${model}${token_meter/usage}${context}${queued}` | Right-aligned bottom status template; cache hit rate remains in the session-statistics row |
