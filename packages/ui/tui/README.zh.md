...
`/accent` 可选择 `deepseek`（默认）、`cosmic-orange`、`mist-blue`、`sage`、`lavender` 和 `deep-blue`。每种色调都同时定义用于强调角色与 banner 渐变的 24 位真彩色，以及用于非真彩色终端的 ANSI 16 色回退，因此两类终端都能保持与明暗主题适配。
