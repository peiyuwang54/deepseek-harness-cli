...
- An optional `allowedDomains` list restricts the initial URL and every same-origin redirect; entries are exact hosts or `*.example.com` subdomain patterns, and a mismatch fails with `WEB_DOMAIN_BLOCKED`.
...
| `allowedDomains` | omitted | Exact hosts or `*.domain` patterns; omitted allows all hosts, `[]` blocks all hosts. |
...
- **The domain allowlist is host-based, not an SSRF defense** — it does not resolve DNS or classify private addresses. Use it as an explicit egress restriction alongside network isolation when fetching untrusted URLs.
