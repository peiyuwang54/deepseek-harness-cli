/** Resolve the native node-pty input used by a single-file runtime builder. */
 ...
 * @param label - product label used in diagnostics.
...
export function resolveLinuxNodePtyAddon(
  ...
  throw new Error(
    `${label}: node-pty addon is absent from both ${built} and ${prebuilt}.`,
  )
}
