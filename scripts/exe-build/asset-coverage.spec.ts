 ...
 * web frontend dist, and native shared libraries) must all be matched by
 * ASSET_GLOBS, or the packaged executable cannot read them from its
 * `/snapshot/` filesystem.
 */
...
beforeAll(async () => {
  ...
  const files: Record<string, string> = {
    ...
    'node_modules/@img/sharp-darwin-arm64/lib/sharp.node': 'addon',
    'node_modules/@img/sharp-libvips-darwin-arm64/lib/libvips-cpp.dylib': 'library',
  }
...
describe('ASSET_GLOBS', () => {
  it('keeps code and data patterns for the staged tree', () => {
    ...
    expect(ASSET_GLOBS).toContain('node_modules/**/*.dylib')
    expect(ASSET_GLOBS).toContain('node_modules/**/*.so*')
    expect(ASSET_GLOBS).toContain('node_modules/**/*.dll')
