 ...
 * POSIX single-file exe or Windows directory runtime under `bin/` and are
 * selected by npm through `os`/`cpu` plus optionalDependencies aliases — the
 * same contract OpenAI Codex uses. The layout is importable so tests can package
 * the host target without a registry; `main()` prints the produced directories.
 */
...
import { WINDOWS_CLI_REQUIRED_RELATIVE_PATHS } from './windows-cli-package.ts'
...
export interface PlatformTarget {
  /** Distribution suffix used in package names and release assets. */
  readonly os: 'macos' | 'linux' | 'windows'
  /** Node/npm platform identifier used by the manifest's `os` field. */
...
export const PLATFORMS: ReadonlyArray<PlatformTarget> = ([
  { os: 'macos', npmOs: 'darwin', cpu: 'arm64' },
  { os: 'macos', npmOs: 'darwin', cpu: 'x64' },
  { os: 'linux', npmOs: 'linux', cpu: 'arm64' },
  { os: 'linux', npmOs: 'linux', cpu: 'x64' },
  { os: 'windows', npmOs: 'win32', cpu: 'arm64' },
  { os: 'windows', npmOs: 'win32', cpu: 'x64' },
] as const).map(target => ({ ...target, name: `${PACKAGE_NAME}-${target.os}-${target.cpu}` }))

const OS_FROM_PLATFORM: Readonly<Record<string, PlatformTarget['os']>> = {
  darwin: 'macos',
  linux: 'linux',
  win32: 'windows',
}
...
function platformManifest(target: PlatformTarget, version: string) {
  return {
    ...
    description: `deepseek-harness-cli runtime for ${target.os}-${target.cpu}`,
    os: [target.npmOs],
    cpu: [target.cpu],
...
/**
 * Copy one target's POSIX exe or Windows runtime directory into a publishable
 * package whose package.json carries the matching `os`/`cpu` fields.
...
export async function layoutPlatformPackage(
  ...
  if (!existsSync(exeSource)) {
    throw new Error(`package-dsh-cli-npm: ${exeSource} missing — stage ${target.os}-${target.cpu} first.`)
  }

  if (target.os === 'windows') {
    const missing = WINDOWS_CLI_REQUIRED_RELATIVE_PATHS.filter(relative => !existsSync(join(exeSource, relative)))
    if (missing.length > 0) {
      throw new Error(`package-dsh-cli-npm: ${exeSource} is incomplete: ${missing.join(', ')} missing.`)
    }
    await cp(exeSource, binDir, { recursive: true })
  } else {
...
function mainManifest(version: string) {
  ...
  return {
    ...
    type: 'module',
    description:
      'deepseek-harness-cli: profile boot, plugin management, and shipped terminal/browser aliases — npm shim over per-platform runtimes',
 ...
 * per-platform runtime, plus optionalDependencies aliases for all six targets.
 * @param outDir - the packaging output root.
 * @param version - the release version, without a leading `v`.
 * @returns the package directory.
 */
...
function usage(): void {
  ...
  --dir <dir>          directory holding staged platform runtimes (default: dist-exe)
