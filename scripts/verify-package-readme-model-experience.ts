...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-permission-presets': { kind: 'indirect', reason: 'The picker submits the host /permissions command; the knob events it appends own the model-visible effect through the sandbox/approval consumers.' },
  ...
  'packages/bundle/tui': { kind: 'indirect', reason: 'The bundle only selects and mounts model-facing capabilities owned by its composed packages.' },
  ...
  'packages/mcp/mcp': { kind: 'indirect', reason: 'The runtime registry delegates model rendering to MCP client tool registrations.' },
  ...
  'packages/subagent/subagent-worktree': { kind: 'none', reason: 'The host-side Git worktree manager registers no model-facing prompt, tool, or request content.' },
